...
use crate::veilid_api::tests::fixtures::*;
use crate::*;
...
pub fn test_veilidappmessage() {
    let orig = VeilidAppMessage::new(
        Some(fix_fake_node_id()),
        Some(fix_fake_routeid()),
        b"Hi there!".to_vec(),
    );
    let copy = deserialize_json(&serialize_json(&orig)).unwrap();
...
pub fn test_safetyspec() {
    let orig = SafetySpec {
        preferred_route: Some(fix_fake_routeid()),
        hop_count: 23,
...
pub fn test_veilidroutechange() {
    let orig = VeilidRouteChange {
        dead_routes: vec![fix_fake_routeid()],
        dead_remote_routes: vec![fix_fake_routeid()],
    };
    let copy = deserialize_json(&serialize_json(&orig)).unwrap();
...
pub fn test_veilidvaluechange() {
    let orig = fix_fake_veilid_value_change();
    let copy = deserialize_json(&serialize_json(&orig)).unwrap();
...
pub fn test_veilidupdate() {
    let orig = VeilidUpdate::ValueChange(Box::new(fix_fake_veilid_value_change()));
    let copy = deserialize_json(&serialize_json(&orig)).unwrap();
