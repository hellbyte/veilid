...
if command -v wasm-opt &> /dev/null; then
  ...
  Install wasm-opt: install binaries via `brew install binaryen` on macos or install from https://github.com/WebAssembly/binaryen/releases/' for other platforms
    exit 1
fi
